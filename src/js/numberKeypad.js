...
(function ($) {
    $.fn.numberKeypad = function (options) {
        ...
        return $this.each(function (i, elem) {
            ...
            var settings = $.extend({
                wrap: 'body',
                arrKeys: [1, 2, 3, 4, 5, 6, 7, 8, 9, 'remove', 0, 'x'],
            ...
            settings.arrKeys.forEach(function (e, i) {
                ...
                switch (e) {
                    case 'remove':
                        strItem += '<a href="javascript:;" class="remove" style="font-size:25px">전체삭제</a>'
                        break;
                    case 'x':
                        strItem += '<a href="javascript:;" class="back"><i class="bi bi-arrow-left" style="padding-top: 10px;"></i></a>'
                        break;
                    default:
            ...
            // 전체 삭제
            $keypad.find("a.remove").on("click", function (e) {
                e.preventDefault();
                for (var i = 0; i < self.val().length; i++) {
                    self.val(self.val().slice(i, 0));
                }
            });
            // click ok event
